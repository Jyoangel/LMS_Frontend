...
export default function EditClass({ params }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push('/teacherspanel/ClassSchedule'); // Redirect after successful delete
    } catch (error) {
  ...
  return (
    <>
      <div className="h-screen w-full flex flex-col p-5 gap-10">
        ...
        {isSelectOpen && (
          <Successcard message="Class schedule successfully updated!" onClose={closeModal} url={"/teacherspanel/ClassSchedule"} />
        )}
      </div>
    </>
  );
